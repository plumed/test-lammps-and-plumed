...
nlines=`paste Sim1/energy1 Sim2/energy2 | tail -n +2 | awk '{if( $2<$4-0.0001 || $2>$4+0.0001 ) print $0}' | wc -l`
if [ "$nlines" -gt 0 ] ; then
   echo ERROR mismatched energies for restrained NVT simuations
   paste Sim1/energy1 Sim2/energy2 | awk '{if( $2<$4-0.0001 || $2>$4+0.0001 ) print $0}'
fi
